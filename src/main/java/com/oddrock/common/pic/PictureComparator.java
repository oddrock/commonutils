 ...
 *
 */
public final class PictureComparator {
	/**
	...
	public PictureComparator(byte[] hashValue) {
	...
	public PictureComparator(String hashValue) {
		this(toBytes(hashValue));
	}

	public PictureComparator(BufferedImage src) {
		this(hashValue(src));
	}
	...
	/**
	 * 从压缩格式指纹创建{@link PictureComparator}对象
	 * 
	 * @param compactValue
	 * @return
	 */
	public static PictureComparator createFromCompact(byte[] compactValue) {
		return new PictureComparator(uncompact(compactValue));
	}
	...
	public boolean equals(Object obj) {
		if (obj instanceof PictureComparator) {
			return Arrays.equals(this.binaryzationMatrix,
					((PictureComparator) obj).binaryzationMatrix);
		} else
	...
	public float compare(String hashValue) {
		return compare(new PictureComparator(hashValue));
	}
	...
	public float compare(BufferedImage image2) {
		return compare(new PictureComparator(image2));
	}
	 ...
	 */
	public float compare(PictureComparator src) {
	...
	public static float compare(BufferedImage image1, BufferedImage image2) {
		return new PictureComparator(image1).compare(new PictureComparator(image2));
	}
	
	public static float compare(String filepath1, String filepath2) throws IOException {
		return new PictureComparator(ImageIO.read(new File(filepath1))).compare(new PictureComparator(ImageIO.read(new File(filepath1))));
	}
